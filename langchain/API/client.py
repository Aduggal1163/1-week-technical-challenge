...
# def get_gemini_ai_response(input_text):
#     response=requests.post("http://localhost:8000/essay/invoke",json={'input':{'topic':input_text}})
#     return response.json()['output']['content']
# #LangServe automatically creates multiple endpoints under /essay.
# #Not just /essay — it creates a full set of endpoints. To actually run the chain → you must use /invoke
# #IMPORTANT In LangServe, /invoke endpoint is used to execute the LLM or chain and get the response. The base route only defines the chain, but /invoke actually runs it and returns the output.
# def get_gemini_ai_response2(input_text2):
#     response=requests.post("http://localhost:8000/poem/invoke",json={'input':{'topic':input_text2}})
#     return response.json()['output']['content']

# st.title("API")
# input_text=st.text_input("Essay")
# input_text2=st.text_input("Poem")

# if input_text:
#     st.write(get_gemini_ai_response(input_text))

# if input_text2:
#     st.write(get_gemini_ai_response2(input_text2))
